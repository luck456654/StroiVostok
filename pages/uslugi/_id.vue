<template>
  <div>
    ...
    <div class="container">
      <h1 :class="{'category-title' : template==4}">{{newArr.main.pagetitle}} {{$store.state.inCity}}</h1>
      <section class="main-section"  v-if="template==4 && showSub=='Да'">
        <Servises :servArr="servArr" :type="'single'" />
      </section>
      <section class="main-section">
        <div v-html="servContent" class="serv-content"></div>
      </section>
    ...
    <TableKrovlya  v-if="$route.params.id == 'krovlya-i-monsarda'"></TableKrovlya>
      <section class="price-section" v-if="template==3"  v-show="false">
        <div class="container">
          ...
          <div class="price-block">
            ...
            <div class="price-block-list">
              <div class="price-block-top-list">
                ...
                <div class="price-block-list-value">
                  Стоимость:
                </div>
              </div>
              <div class="price-block-list-item" v-for="(item,index) in arrServType[activeCategory].child" :key="index">
                <div class="price-block-list-name">
                  {{item.name}}
                </div>
                <div class="price-block-list-value">
          ...
          <div class="btn-standart btn-work">
            Рассчитать стоимость работ
          </div>
        </div>
      </section>
      <div class="container">
        <div class="temp-content" v-if="!servContent">
    ...
    <section class="work-block" v-if="(template==3 || template==4) && worksArr.length>0">
      <div class="container">
        <h3>Наши работы</h3>
        <div class="work-row">
...
export default {
  components: { Bredcrumbs, Calc, Servises, Work, Request, CalcGroundWork, TablePrices, TableKrovlya },
   ...
   data() {
    return {
      pagetitle: "СТРОЙ ВОСТОК: строительные и ремонтные услуги " + this.$store.state.inCity,
      title:'',
      seoTitle: "",
      description: "",
      activeCategory: 0,
      ...
      arrServType:[
        {name:'Отделочные работы', active:true, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ...
        {name:'Электромонтажные работы', active:false, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ...
        {name:'Сантехнические работы', active:false, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ]},
        {name:'Столярные работы', active:false, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ]},
        {name:'Демонтажные работы', active:false, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ...
        {name:'Прочие работы', active:false, child:[
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
          {name:'Демонтаж подвесного шкафа (тип - антресоль)', price:'1600'},
        ]},
  ...
  head() {
    return {
      title: this.newArr.main.seotitle ? this.newArr.main.seotitle + ' ' +this.$store.state.inCity : this.title + " СТРОЙ ВОСТОК " + this.$store.state.inCity,
      meta: [
        {
          hid: "description",
          name: "description",
          content: this.newArr.main.seodescription ? this.newArr.main.seodescription + ' ' +this.$store.state.inCity : this.title + " СТРОЙ ВОСТОК: строительные и ремонтные услуги " +  this.$store.state.inCity,
        },
      ],
    };
  },
  async asyncData({ $axios, error, route, store }) {
    ...
    return await $axios.$post(store.state.apiDomen + `/api/main/usluga.php`, data)
      .then((arr) => {
        console.log(arr)
        return {
          newArr: arr
  ...
  async mounted(){
    ...
    this.title =  this.newArr.main.longtitle ? this.newArr.main.longtitle : this.newArr.main.pagetitle
    this.template = this.newArr.main.template
    this.servContent = this.newArr.main.content
    this.requestName = this.newArr.main.quoting ? this.newArr.main.quoting : 'Нужен ремонт<br>или строительство<br>дома?'
...
.price-block-list-item, .price-block-top-list{
  flex-wrap: wrap;
  display: flex;
  align-items: center;
}
